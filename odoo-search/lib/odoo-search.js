...
program
    ...
    .option('-q, --query <DOMAINS>', 'JSON-encoded list of Odoo domains to apply. If unspecified, all records are returned.', (val) => val.split(','))
    .option('-f, --fields <FIELD1[,...]>', 'Comma-separated list of fields to return. If unspecified, all fields are returned.', (val) => val.split(','))
    .version(pkg.version)
    .parse(process.argv);
